...
/*   Updated: 2020/06/23 11:46:50 by avan-ber      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_rotate(double rot, t_info *info)
{
	...
	info->ray.dir.x = (info->ray.dir.x * cos(rot)) -
												(info->ray.dir.y * sin(rot));
	info->ray.dir.y = (old_dir_x * sin(rot)) + (info->ray.dir.y * cos(rot));
	old_plane_x = info->ray.plane.x;
	info->ray.plane.x = (info->ray.plane.x * cos(rot)) -
												(info->ray.plane.y * sin(rot));
	info->ray.plane.y = (old_plane_x * sin(rot)) +
												(info->ray.plane.y * cos(rot));
}
...
void	ft_move_left(t_info *info)
{
	...
	if (info->ray.map[pot_pos.y][pot_pos.x] == 0)
